...
public class BluetoothMessageService {
	...
	// Bluetooth UART UUID
	private static final UUID BLUETOOTH_UART_UUID = UUID
	...
	private synchronized void setState(int state) {
		...
		mHandler.obtainMessage(MessageHandler.MESSAGE_STATE_CHANGE, state, -1)
				.sendToTarget();
	}
	...
	public synchronized void connected(BluetoothSocket socket,
		...
		final Message msg = mHandler
				.obtainMessage(MessageHandler.MESSAGE_DEVICE_NAME);
	...
	private void connectionFailed() {
		...
		final Message msg = mHandler
				.obtainMessage(MessageHandler.MESSAGE_TOAST);
		final Bundle bundle = new Bundle();
		...
		public AcceptThread() {
			...
			try {
				tmp = mAdapter.listenUsingRfcommWithServiceRecord(
						SDP_RECORD_NAME, BLUETOOTH_UART_UUID);
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			...
			try {
				tmp = device
						.createRfcommSocketToServiceRecord(BLUETOOTH_UART_UUID);
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					if (frame != null) {
						Log.i(TAG,
								"Sending frame back of length: " + frame.size());
						mHandler.obtainMessage(MessageHandler.MESSAGE_READ,
								frame.size(), -1, frame).sendToTarget();
					}
					// Send the stream to a parser will write to the screen if
					// need be.
					// msg.echoBack(buffer);
					// Send the obtained bytes to the UI Activity
					// mHandler.obtainMessage(LampAlarmMain.MESSAGE_READ, bytes,
					// -1, buffer)
					// .sendToTarget();
		...
		public void write(byte[] buffer) {
			try {
				...
				mHandler.obtainMessage(MessageHandler.MESSAGE_WRITE, -1, -1,
						buffer).sendToTarget();
			} catch (final IOException e) {
